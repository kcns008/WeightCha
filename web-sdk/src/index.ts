...
export class WeightCha {
  private config: WeightChaConfig;
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to initialize WeightCha: ${(error as Error).message}`);
    }
  }

  async verify(containerId: string, options: ChallengeOptions = {}): Promise<VerificationResult> {
    if (!this.initialized) {
      await this.init();
    }
  ...
  private async checkClientAvailability(): Promise<void> {
  ...
  private async createChallenge(options: ChallengeOptions): Promise<Challenge> {
  ...
  private renderUI(container: HTMLElement, challenge: Challenge, options: ChallengeOptions): void {
    container.innerHTML = `
  ...
  private bindEvents(container: HTMLElement, challenge: Challenge, options: ChallengeOptions): void {
    const startBtn = container.querySelector('#weightcha-start') as HTMLButtonElement;
    const cancelBtn = container.querySelector('#weightcha-cancel') as HTMLButtonElement;
  ...
  private async waitForVerification(challenge: Challenge): Promise<VerificationResult> {
...
class Challenge {
  ...
  private async startMacOSChallenge(): Promise<void> {
  ...
  private startBrowserChallenge(): void {
    // Set up browser-based pressure simulation
    const pressureArea = document.querySelector('.weightcha-status') as HTMLElement;
    if (!pressureArea) return;
    ...
    const startTracking = (e: Event) => {
      ...
      // For touch devices, try to use force if available
      const getTouchForce = (event: any) => {
  ...
  private async completeBrowserChallenge(pressureData: any[]): Promise<void> {
    try {
  ...
  private analyzePressurePattern(data: any[]): { isHuman: boolean; confidence: number } {
    if (data.length < 5) return { isHuman: false, confidence: 0 };
  ...
  private updateProgress(progress: number): void {
    const progressFill = document.querySelector('.weightcha-progress-fill') as HTMLElement;
    if (progressFill) {
...
if (typeof window !== 'undefined') {
  (window as any).WeightCha = WeightCha;
}
